...
void Single_Cycle_CPU(){
    ...
    while (true)  //Note: Use while loop to simulate the clock
    {
       ...
       /* Program Counter */
        ...
        bitset<1> MemRead_2=CTRL.get_MemRead();
        bitset<1> MemtoReg=CTRL.get_MemtoReg();
        bitset<2> ALUop=CTRL.get_aluop();
        bitset<1> Memwrite_2=CTRL.get_MemWrite();
        ...
        bitset<6> alu_ctrl_in = i_function_code;
        bitset<2> alu_ctrl_in1 = ALUop;
        ...
        bitset<1> MemWrite = Memwrite_2;
        bitset<1> MemRead = MemRead_2;
        ...
        /* Find the next PC value */
         ADDER_PC.input(pc_out, 4); //what are the input for this adder?
        ...
        ANDGATE.input(alu_zero, Branch); // what is the input for the ANDgate
